"""
DeepSeek服务：调用DeepSeek大模型API进行文本生成。
"""
import os
import requests
import json
...
def call_deepseek(message, model_config, api_url=None, model_name=None):
    """
    调用DeepSeek API生成文本。
    ...
    if not api_key:
        return "错误：DeepSeek API密钥未配置！", 0, 0, 0
    
    # 使用传入的URL或默认URL
    url = api_url or os.getenv("DEEPSEEK_API_URL", "https://api.deepseek.com/v1/chat/completions")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                pass
            return f"错误：API请求失败，状态码: {response.status_code}{error_detail}", total_time_ms, total_time_ms, 0
...
def call_deepseek_v4(message, model_config, api_url=None, model_name=None, context_messages=None, api_key=None):
    """
    调用DeepSeek-V4-Flash API生成文本。
    ...
    if not api_key:
        return "错误：DeepSeek-V4-Flash API密钥未配置！", 0, 0, 0
    
    # 使用传入的URL或默认URL
    url = api_url or os.getenv("DEEPSEEK_V4_API_URL", "https://api.deepseek.com/v1/chat/completions")
    
    # 使用传入的模型名称或默认名称
    model = model_name or os.getenv("DEEPSEEK_V4_MODEL_NAME", "deepseek-chat")
